...
if ($uploadOk == 0) {
  ...
  $random = base_convert(rand(),10,36) . date("his");
	...
	move_uploaded_file($_FILES["fileImage"]['tmp_name'], $path . $pic_name);
  
  //Save image
	if (true !== ($pic_error = @image_resize($path . $pic_name, $path . "$random$pic_type", 1600, 1600))) { //Resize image to max 1600x1600 to safe size
		echo "<html><body onload=\"alert('".$pic_error."')\"></body></html>";		
		exit();
	}
  
  unlink($path . $pic_name); //delete original
  
  //Replace image src with the new saved file
  echo "<html><body onload=\"parent.applyLargerImage('" . $path . "$random$pic_type" . "')\"></body></html>";
